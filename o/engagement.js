function getdata(survey_num, type) {
    ...
    var settings = {
        "url": backendURL+"/Oengagement/"+type,
        "method": "POST",
        "timeout": 0,
        "headers": {
    ...
    $.ajax(settings).done(function (response) {
        engagement(response.engagement.year, response.engagement.engagement);
...
function engagement(year, engagement) {
    engagement = engagement.map(function(each_element){
        return Number(each_element.toFixed(2));
    });
    echarts.init(document.querySelector("#engagement")).setOption({
        tooltip: {
            trigger: 'axis',
            axisPointer: {
                type: 'shadow'
