function getdata(survey_num, type) {
    ...
    var settings = {
        "url": backendURL+"/Omasculinity/"+type,
        "method": "POST",
        "timeout": 0,
        "headers": {
    ...
    $.ajax(settings).done(function (response) {
        masculinity(response.masculinity.year, response.masculinity.masculinity);
...
function masculinity(year, manbox) {
    manbox = manbox.map(function(each_element){
        return Number(each_element.toFixed(2));
    });
    echarts.init(document.querySelector("#masculinity")).setOption({
        tooltip: {
            trigger: 'axis',
            axisPointer: {
                type: 'shadow'
